...
export default function Dashboard() {
    ...
    function filterByPeriod(selectedPeriod){
        if (selectedPeriod === "all") {
            ...
            setFilteredOrders(orders.filter((order) => getMyWeek(order.day) === getWeek()))
        }
    }
    ...
    return(
        <div className="flex flex-col items-center content-center justify-start w-full min-h-screen gap-4 px-2 pt-10 md:px-10">
            <h3 className="text-3xl font-bold">ORDERS</h3>
            
            <div className="flex flex-row items-center w-full gap-4">
                <div role="tablist" className="p-2 tabs tabs-boxed rounded-xl">
                    <a role="tab" className={"tab " + (selectedPeriod==="all" && " tab-active")} onClick={() => filter("all")}>All</a>
                    <a role="tab" className={"tab " + (selectedPeriod==="today" && " tab-active")} onClick={() => filter("today")}>Today</a>
                    <a role="tab" className={"tab " + (selectedPeriod==="week" && " tab-active")} onClick={() => filter("week")}>This week</a>
                </div>
                <span className="text-lg font-semibold opacity-50 ">{filteredOrders.length} orders</span>
            </div>
